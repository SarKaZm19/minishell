...
const char *token_type_to_string(t_token_type type) {
    switch (type) {
		...
		case TK_GROUP_OPEN: return "TK_GROUP_OPEN";
		case TK_GROUP_CLOSE: return "TK_GROUP_CLOSE";
...
void command_to_json(t_ast_command *command, char **json_str) {
    strcat(*json_str, "{\"type\": \"command\", \"cmd_exe\": [");
    for (int i = 0; command->cmd_exe[i] != NULL; i++) {
...
// // Not used because it break the ms-tester
// void command_to_json(t_ast_command *command, char **json_str) {
//     strcat(*json_str, "{\"type\": \"command\", \"content\": \"");
//     for (int i = 0; command->cmd_exe[i] != NULL; i++) {
//         if (i > 0) strcat(*json_str, " "); // Add space between arguments
//         escape_and_append(json_str, command->cmd_exe[i]);
//     }
//     strcat(*json_str, "\"}");
// }
...
void subshell_to_json(t_ast_group *group, char **json_str) {
	strcat(*json_str, "{\"type\": \"group\", \"child\": ");
	ast_to_json(group->child, json_str);
	strcat(*json_str, "}");
}
...
void ast_to_json(t_ast *node, char **json_str) {
    ...
    switch (node->type) {
        case AST_COMMAND:
            command_to_json(&node->data.command, json_str);
            break;
        case AST_PIPELINE:
            pipeline_to_json(&node->data.pipeline, json_str);
            break;
        case AST_LOGICAL:
            logical_to_json(&node->data.logical, json_str);
            break;
		case AST_GROUP:
			subshell_to_json(&node->data.group, json_str);
			break;
		case AST_REDIRECTION:
            redirection_to_json(&node->data.redirection, json_str);
            break;
		default:
			break;
